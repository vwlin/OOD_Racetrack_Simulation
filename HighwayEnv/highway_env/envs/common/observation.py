...
class OccupancyGridObservation(ObservationType):
    ...
    def observe(self, noisy=False, b=1.0) -> np.ndarray:
        if not self.env.road:
            return np.zeros(self.space().shape)
            ...
            for layer, feature in enumerate(self.features):
                if feature in df.columns:  # A vehicle feature
                    for _, vehicle in df[::-1].iterrows():
                        ...
                        '''*******************************************
                        Begin Failure Prediction augmentation...
                        *******************************************'''
                        if noisy:
                            x += np.random.uniform(low=-b, high=b, size=x.shape)
                            y += np.random.uniform(low=-b, high=b, size=y.shape)
                        '''*******************************************
                        End Failure Prediction augmentation...
                        *******************************************'''
                    ...
                    Begin Failure Prediction augmentation...
                    *******************************************'''
                    self.fill_road_layer_by_lanes(layer, noisy=noisy, b=b)
                    '''*******************************************
                    End Failure Prediction augmentation...
                    *******************************************'''
        ...
        for _from in road.network.graph.keys():
            for _to in road.network.graph[_from].keys():
                for lane in road.network.graph[_from][_to]:
                    ...
                    *******************************************'''
                    if noisy:
                        position += np.random.uniform(low=-b, high=b, size=position.shape)
                    '''*******************************************
                    End Failure Prediction augmentation...
                    *******************************************'''
                    origin, _ = lane.local_coordinates(position)
                    waypoints = np.arange(
